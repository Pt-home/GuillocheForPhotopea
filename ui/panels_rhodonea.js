// /ui/panels_rhodonea.js — Rhodonea (Rose) UI panel

export function mountRhodoneaPanel(root, store){
  root.innerHTML = '';
  ...
  // Shape
  const g1 = group('Rhodonea (Rose)');
  g1.append(
    rowNum('A (radius)', p.A ?? 140, 1, 5000, 1, v => store.updatePathParams({ A:v })),
    rowNum('k (freq)',   p.k ?? 5, 0.5, 256, 0.5, v => store.updatePathParams({ k:v })),
    rowNum('phi (rad)',  p.phi ?? 0, -6.283, 6.283, 0.01, v => store.updatePathParams({ phi:v })),
    rowSelect('variant', p.variant || 'cos', ['cos','sin'], v => store.updatePathParams({ variant:v }))
  );
