// /ui/panels_clothoid.js — UI for Clothoid (Euler spiral)

export function mountClothoidPanel(root, store){
  root.innerHTML = '';
  ...
  const rowChk = (label, checked, onChange)=>{
    const wrap=document.createElement('div'); wrap.className='row';
    const lab=document.createElement('label'); lab.textContent=label;
  ...
  const g1 = group('Clothoid (Euler spiral)');
  g1.append(
    rowNum('t0', p.t0 ?? 0, -10, 10, 0.01, v => store.updatePathParams({ t0: v })),
    rowNum('t1', p.t1 ?? 3.5, -10, 10, 0.01, v => store.updatePathParams({ t1: v })),
    rowChk('Symmetric S-curve', (p.symmetric ?? true), v => store.updatePathParams({ symmetric: v })),
    rowNum('Scale (px)', p.scale ?? 180, 1, 10000, 1, v => store.updatePathParams({ scale: v })),
    rowNum('Rotate (rad)', p.rotate ?? 0, -Math.PI*2, Math.PI*2, 0.01, v => store.updatePathParams({ rotate: v })),
    rowSel('Direction', (p.dir ?? 1).toString(), ['1','-1'], v => store.updatePathParams({ dir: parseInt(v,10) }))
  );
