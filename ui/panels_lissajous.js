// /ui/panels_lissajous.js — Lissajous UI panel

export function mountLissajousPanel(root, store){
  root.innerHTML = '';
  ...
  // Shape
  const g1 = group('Lissajous');
  g1.append(
    rowNum('A (x amp)', p.A ?? 120, 1, 5000, 1, v => store.updatePathParams({ A:v })),
    rowNum('B (y amp)', p.B ?? 120, 1, 5000, 1, v => store.updatePathParams({ B:v })),
    rowNum('a (x freq int)', p.a ?? 3, 1, 512, 1, v => store.updatePathParams({ a: Math.max(1, Math.round(v)) })),
    rowNum('b (y freq int)', p.b ?? 2, 1, 512, 1, v => store.updatePathParams({ b: Math.max(1, Math.round(v)) })),
    rowNum('delta (rad)', p.delta ?? 0, -6.283, 6.283, 0.01, v => store.updatePathParams({ delta:v }))
  );
  ...
  // This panel does not attach store subscriptions itself (no dynamic lists),
